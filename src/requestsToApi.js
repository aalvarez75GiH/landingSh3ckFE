...
export const verifyingTokenRequest = async(token) => {
    return await axios.get(`${URL_LOCAL_BACKEND}/api/users/me`, {
    headers:{
...
export const addingGoogleUserToDB = async(token) => {
    return await axios.post(`${URL_LOCAL_BACKEND}/api/extUsers/google`, {
        token,
        headers:{
...
export const addingInterestedUserToDB = async(interestedUser) => {
    return await axios.post(`${URL_LOCAL_BACKEND}/api/interestedUsers`, interestedUser)
}

export const requestToLoginUsers = async(user) => {
    return await axios.post(`${URL_LOCAL_BACKEND}/api/users/login`, user)
}

export const addingRegularUsersToDB = async(user) => {
    return await axios.post(`${URL_LOCAL_BACKEND}/api/users`, user)
}
...
export const getRequestToCategories = async() => {
    const token = localStorage.getItem("SH3CK_TOKEN")
    console.log(token)
    const response = await axios.get(`${URL_LOCAL_BACKEND}/api/category`, {
        headers:{
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`   
        }, 
...
export const getRequestToServiceTime = async() => {
    const token = localStorage.getItem("SH3CK_TOKEN")
    console.log(token)
    const response = await axios.get(`${URL_LOCAL_BACKEND}/api/times`, {
        headers:{
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`   
        }, 
